...
fn test_fitness_zararli_model_negatif() {
    ...
    assert!(f < 0.0 && f > -1_000_000.0, "Zararlı model skoru anormal: {}", f);
}
...
fn test_random_genome_clamping() {
    ...
    for i in 128..131 {
        assert!(g.weights[i] >= -1.0 && g.weights[i] <= 1.0, 
            "Action threshold {} sınırlar dışında: {}", i, g.weights[i]);
    }
...
fn test_evolve_elite_korunur() {
    ...
    for i in 0..10 {
        assert!(new_pop[i].fitness == f64::NEG_INFINITY || 
                (new_pop[i].weights == pop[i].weights),
            "Elite genom {} değişmiş", i);
    }
}
...
fn test_evolve_clamping_korunur() {
    ...
    for g in &new_pop {
        assert!(g.weights[131] >= DNA_TP_MIN && g.weights[131] <= DNA_TP_MAX,
            "TP clamp başarısız: {}", g.weights[131]);
        assert!(g.weights[132] >= DNA_SL_MIN && g.weights[132] <= DNA_SL_MAX,
            "SL clamp başarısız: {}", g.weights[132]);
        assert!(g.weights[133] >= DNA_COOLDOWN_MIN && g.weights[133] <= DNA_COOLDOWN_MAX,
            "Cooldown clamp başarısız: {}", g.weights[133]);
    }
}
...
fn test_farkli_seed_farkli_genom() {
    ...
    let fark_var = g1.weights.iter().zip(g2.weights.iter()).any(|(a, b)| a != b);
...
fn test_fitness_landscape_siralama() {
    ...
    assert!(iyi > orta, "İyi skor ortadan düşük: iyi={}, orta={}", iyi, orta);
    assert!(orta > kotu, "Orta skor kötüden düşük: orta={}, kotu={}", orta, kotu);
    assert!(kotu < -20_000_000.0, "Kötü model aşırı ceza almadı: {}", kotu);
}
...
fn test_ablation_sharpe_bonus_etkisi() {
    ...
    assert!((diff - expected).abs() < 1.0, 
        "Sharpe ablation beklenenden farklı: diff={}, expected={}", diff, expected);
}
...
fn test_ablation_profit_factor_bonus_etkisi() {
    ...
    assert!((diff - expected).abs() < 1.0,
        "PF ablation beklenenden farklı: diff={}, expected={}", diff, expected);
}
...
fn test_ablation_activity_bonus_etkisi() {
    ...
    assert!((diff - expected).abs() < 1.0,
        "Activity ablation beklenenden farklı: diff={}, expected={}", diff, expected);
}
...
fn test_ablation_pnl_penalty_vs_reward() {
    ...
    assert!(pos > neg.abs(), "Negatif PnL cezası pozitif ödülünden büyük olmamalı");
}
...
fn test_overfit_az_trade_cezasi() {
    ...
    assert!(overfit < -20_000_000.0, 
        "Overfit model (20 trade) korkaklık cezası almadı: {}", overfit);
    
    assert!(generalist > 0.0, 
        "Genelleştiren model negatif skor aldı: {}", generalist);
}
...
fn test_multi_generation_elite_korunur() {
    ...
    assert!(final_best >= initial_best, 
        "Fitness düştü: {} -> {}", initial_best, final_best);
}
...
fn test_evolution_convergence_demo() {
    ...
    for gen in 0..GENERATIONS {
        ...
        if gen < GENERATIONS - 1 {
            pop = evolve_population_seeded(&pop, POP_SIZE, 0.15, false, SEED_BASE + 1000 + gen as u64);
        }
    }
    ...
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  İlk Gen Best Fitness: {:>12.2}                                 ║", first_best);
    println!("║  Son Gen Best Fitness: {:>12.2}                                 ║", last_best);
    println!("║  İyileşme:             {:>11.2}%                                ║", improvement);
    println!("╚══════════════════════════════════════════════════════════════════╝");
    
    assert!(
        last_best >= first_best,
        "Evrim çalışmıyor! Fitness düştü: {} -> {}", first_best, last_best
    );
    ...
    assert!(
        ...
        "Popülasyon ortalaması iyileşmedi: {} -> {}", first_5_avg, last_5_avg
    );
}